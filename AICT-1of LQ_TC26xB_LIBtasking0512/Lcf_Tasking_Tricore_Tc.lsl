...
derivative tc26
{
	...
	section_layout :vtc:linear
	{
		...
		group (ordered, attributes=rwx, copy, run_addr=mem:psram0)
		{
			/* The performance-dependent functions called in cpu0 can be placed in psram0 */
//			select ".text.Cpu0_Main.armFunTest";
		...
		group (ordered, attributes=rwx, copy, run_addr=mem:psram1)
		{
			/* The performance-dependent functions called in cpu1 can be placed in psram1 */
		}
		

		group (ordered, contiguous, align = 4, run_addr = mem:dsram0)
		{
			/* The data invoked in cpu0 can be placed in dsram0 */
//			select ".bss.Cpu0_Main.g_AppCpu0";

//			select ".bss.LQ_CAMERA.Image_Data";
//			select ".bss.LQ_CAMERA.Image_Use";
		...
		{
			/* The data invoked in cpu1 can be placed in dsram1 */
...
//			select ".bss.Cpu0_Main.ramTestBuff";
		}
	}
	/*Near data sections*/
